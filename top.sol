...
contract Top is IERC20, Ownable {
    ...
    constructor(
        ...
        address ReceiveAddress = address(0x0770d5C83b20a4024879CE97364f351E1B820C8b);
        ...
        holderRewardCondition = 50 ** IERC20(currency).decimals() ;
        holderRewardConditionDD = 50 ** IERC20(currency).decimals() ;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (swapAmount > 0) {
                ...
                _ts.rewardAmount += tAmount*tsRewardFee/10000;
            }
            ...
            if (!isSell) {
                ...
                _takeTransfer(sender, address(1), burnAmount);
                
            } else {
                burnAmount = (tAmount * sell_burnFee) / 10000;
                if(balanceOf(address(1))+balanceOf(address(0xdead))<dead_threshold){
                    feeAmount += burnAmount;
                    _takeTransfer(sender, address(1), burnAmount);
                }else{
                    feeAmount += burnAmount;
        ...
        if (recipient==address(this) &&!isContract(sender) && sender!=owner()){
            ...
            if(usdtThis>=usdtAmount){
                ...
                IERC20(currency).transfer(sender,usdtAmount); //托底池价格,扣1%手续费，待确认
                feeAmount+=tAmount;
            }
    ...
    function swapTokenForFund(
        ...
        if (newBal != 0) {
            ...
            uint256 _rewardAmount = newBal * _ts.rewardAmount / (_ts.rewardAmount+_ts.profitAmount);
            uint256 _profitAmount = newBal * _ts.profitAmount / (_ts.rewardAmount+_ts.profitAmount);

            if(_profitAmount>0){
                _c.transfer(address(_deadDistributor), _profitAmount*50/100);
            }
    ...
    function processReward(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > 10*10**18 && !excludeHolder[shareHolder]) {
                ...
                amount2 = balance*lpAmount[shareHolder]/lpAmountAll;
                if(amount2<amount){
                    amount = amount2;
                }
